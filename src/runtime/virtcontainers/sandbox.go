...
func (s *Sandbox) prepareInitdataMount(config *HypervisorConfig) error {
	if len(config.Initdata) == 0 {
		s.Logger().WithField("initdata", "No initdata provided. Skip prepare initdata device").Info("No initdata provided. Skip prepare loop device")
		return nil
	}
	...
	rw, err := fs.OpenFile("/initdata.toml", os.O_CREATE|os.O_RDWR)
	if err != nil {
		s.Logger().WithError(err)
		return err
	}

	s.Logger().Debugf("initdata from runtime siede: %s", config.Initdata)
